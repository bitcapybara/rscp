...
impl ProtocolStream {
    pub async fn method_recv(&mut self) -> Result<Method> {
        match self.method_decode().await {
            Ok(method) => {
                self.as_mut().write_all(&[0u8; 1]).await?;
    ...
    async fn method_decode(&mut self) -> Result<Method> {
        // magic number
        if self.as_mut().read_u16().await? != MAGIC_NUMBER {
            return Err(Error::MissMagicNumber);
        }
        // action
        match self.as_mut().read_u8().await? {
            0x01 => {
                // check path exists
    ...
    async fn method_decode_path(&mut self) -> Result<PathBuf> {
        let path_len = self.as_mut().read_u16().await? as usize;
        let mut path_bytes = Vec::from_iter(std::iter::repeat(0u8).take(path_len));
        self.as_mut().read_exact(&mut path_bytes).await?;
    ...
    async fn assert_reply(&mut self) -> Result<()> {
        if self.as_mut().read_u8().await? != 0 {
            let msg_len = self.as_mut().read_u16().await? as usize;
            let mut msg_bytes = vec![0u8; msg_len];
            self.as_mut().read_exact(&mut msg_bytes).await?;
    ...
    pub async fn recv_file(&mut self, path: PathBuf) -> Result<()> {
        ...
        while writed < metadata.file_size {
            let read = self.as_mut().read(&mut *buf).await?;
        ...
        if writed != self.as_mut().read_u64().await? {
        ...
        self.as_mut().read_exact(&mut *checksum_bytes).await?;
    ...
    async fn decode_meta(&mut self) -> Result<FileMeta> {
        ...
        // is dir
        let is_dir = self.as_mut().read_u8().await? == 1;
        if is_dir {
            return Ok(FileMeta {
                path,
                is_dir,
        ...
        // permission
        let permission = self.as_mut().read_u32().await?;
        // file size
        let file_size = self.as_mut().read_u64().await?;
